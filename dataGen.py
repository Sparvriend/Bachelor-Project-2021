...
# The paper that is constantly mentioned in this file and relates to the other python files:
...
# Constant for the in out patient distance, measured in km. Copied directly from the paper by Bench-Capon.
IN_OUT_PATIENT_DISTANCE = 50
NOISE_VARIABLES = 52

# This function initializes data generation.
# The printFailOn function can be uncommented to print what rules were failed on for the training datasets.
...
# This function adds noise to a dataset and converts it to an excel file for manual viewing.
def modifyData(tf, DATA_POINTS, TYPE, LOC):
    tf = pd.concat([tf, generateNoiseData(DATA_POINTS)], axis = 1)
    tf.to_excel(LOC + TYPE + '.xlsx')
    return tf

# Function that shows what rules caused the patients to not receive the welfare benefit. Can be used for any dataset.
...
# Generating numerical data, based on a list of values that can be chosen from.
def generateNumerical(min, max, DATA_POINTS, step = 1):
    sample = list(range(min, max+1, step)); data = []
...
def generateNoiseData(DATA_POINTS):
    ...
    return pd.DataFrame.from_records(list(map(list, zip(*noiseVariables))), columns = keys)

# Function that generates a list of size DATA_POINTS, with all datapoints being eligible.
def generatePerfectData(DATA_POINTS):
    ...
    # Ensuring that a contribution can be failed on, contributions > 3 (out of 5 contribution years).
    for i in range(len(df.Age)):
        contrYears = [1,1,1,1,1]; yearsPaid = random.choice(list(range(0, 2)))
    ...
    return pd.concat([df, pd.DataFrame({'Eligible': eligibility})], axis = 1)
